...
AV.Cloud.afterUpdate("Reviews", async function (request) {
  ...
  if (!updatedKeys.includes("approved")) {
    console.log(
      "Not modifying approved field, skip review after update hook..."
    );
    return;
  }
  console.log("change approval state: " + currReviewState.get("approved"));
  const isApproved = currReviewState.get("approved");
  if (!isApproved) {
    console.log("Not approved, skip review after update hook...");
  }
